...
TemporaryIdentifierWidgetController = (function() {
  ...
  TemporaryIdentifierWidgetController.prototype.on_value_change = function(event) {
    ...
    return this.search_mrn(el.value, catalog_name).done(function(data) {
      ...
      // Construimos el "record" con subcampos explícitos (4 partes) y MRN
      record = {
        // (opcional) cadena plana para compatibilidad
        "PatientFullName": (data.Title || ""),
        // Subcampos del nombre completo para el FullnameWidget (4 partes)
        "PatientFullName.firstname": (data.firstname || ""),
        "PatientFullName.middlename": (data.middlename || ""),
        "PatientFullName.lastname": (data.lastname || ""),
        "PatientFullName.maternal_lastname": (data.maternal_lastname || ""),
        // MRN para el AR (ObjectField -> subcampo 'value')
        "MedicalRecordNumber.value": (data.mrn || ""),
        // Resto de campos informativos
  ...
  /*
  Sets the temporary value of the field
   */
  ...
  /*
  Sets the value for a sibling field with specified base name
   * Soporta subcampos con nombre punteado, p.ej.:
  ...
  TemporaryIdentifierWidgetController.prototype.set_sibling_value = function(element, name, value) {
    ...
    // Si es un subcampo (base.subcampo), usa get_subfield y respeta arnum
    if (name.indexOf(".") > -1) {
      var parts = name.split(".");
      var base = parts[0];         // p.ej. "PatientFullName" o "MedicalRecordNumber"
      var sub = parts.slice(1).join("."); // "firstname" / "value" / etc.

      var parent = element.closest("td[arnum]") || element.closest("div[data-fieldname]");
      var sample_num = parent && (parent.getAttribute("arnum") || $(parent).attr("arnum"));
      var fieldbase = base + (this.is_add_sample_form && sample_num ? ('-' + sample_num) : '');

      var input = this.get_subfield(fieldbase, sub);
      if (input) {
        this.debug(">>> " + input.name + " = " + value + " ");
        input.value = value;
      }
      return;
    }
    ...
    // Campo simple
    var field = this.get_sibling(element, name);
    if (!field) {
      return;
    }
  ...
  TemporaryIdentifierWidgetController.prototype.search_mrn = function(mrn, catalog_name) {
    ...
    // Pedimos también los 4 subcampos del nombre y el MRN
    fields = [
      "Title",
      "firstname", "middlename", "lastname", "maternal_lastname",
      "mrn",
      "age", "birthdate", "sex", "gender",
  ...
  /*
  Render the content of a Handlebars template in a jQuery UI dialog
   */
